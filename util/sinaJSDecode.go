...
var (
	decoder  *goja.Runtime
	evalLock sync.Mutex
)

func init() {
	decoder = goja.New()
	_, err := decoder.RunString(sinaTemplate)
	if err != nil {
		panic(err)
	}
}
...
func DecodeSinaData(rawData string) (interface{}, error) {
	vm := goja.New()
	vm.RunString(sinaTemplate)

	var fn func(string) []map[string]interface{}
	if err := vm.ExportTo(vm.Get("d"), &fn); err != nil {
		return nil, err
	}
	result := fn(rawData)
	return result, nil
