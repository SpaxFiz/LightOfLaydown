...
func GetPECrawlToken() (string, error) {
	code := strings.ReplaceAll(jscode, "${date}", time.Now().Format("2006-01-02"))
	vm := otto.New()
	if script, err := vm.Compile("", code); err != nil {
		logrus.Errorf("GetPECrawlToken javascript eval failed. err=%s", err.Error())
		return "", err
	} else {
		result, e := vm.Run(script)
		if e != nil {
			return "", nil
		}
