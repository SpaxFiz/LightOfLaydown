...
function resolvePEData(data: PEArray): SeriesData {
  ...
  data.forEach(item => {
    if (item === null) {
      ...
      result[4].push(parseFloat((item.weighted_pe_ttm_quantile * 100).toFixed(2)))
    }
  })
  return result
}
...
function genSeries(data: SeriesData, prefix: indexName, m: mode): object | object[] {
  let color = ''
  ...
  'methods': {
    ...
    constructData(data: BackendData): EChartsOption {
      let result = prepareOptions(data),
        ...
        yAxisOpt: echarts.YAXisComponentOption = {},
    ...
    switchMode(m: string) {
      ...
      let newOption = this.constructData(this.rawData)
...
<template>
  ...
  <div id="chartPE" v-loading="loading"></div>
</template>
